---
name: ui-ux-pro-max
description: "UI/UX design intelligence for web, mobile, and desktop, backed by a searchable local database (BM25) and a design-system generator. Use when designing, building, reviewing, or fixing interfaces: pages, components, landing pages, dashboards, design systems, accessibility, interaction, responsive layout, typography, color palettes, charts, animations, or stack-specific UI implementation. Triggers on requests like 做一个落地页 / 设计仪表板 / 生成设计系统 / 选配色和字体 / UI 风格推荐 / 美化界面 / review my UI / build a page. Data: 79 searchable styles (50 active), 192 product palettes and reasoning profiles, 74 font pairings, 119 UX guidelines, 105 icons, 17 GSAP presets, 25 chart types, 22 stacks."
---
...
The search script lives inside **this skill's own directory**, not the project directory. In this document `<skill-dir>` means the base directory of this skill — the absolute path of the folder containing this SKILL.md (ZCode reports it when the skill loads; it is the folder that contains `scripts/search.py`, `data/`, and `references/`). Always invoke the script by its full absolute path:

```bash
python "<skill-dir>/scripts/search.py" "<query>" --domain <domain>
```

If `python` is not found, try `py -3`, then `python3`. Requires Python 3.x, standard library only — no external dependencies, no network requests.
...
This aggregates product/style/color/landing/typography matches, applies reasoning rules from `data/ui-reasoning.csv`, and returns pattern, style, colors, typography, effects, and anti-patterns to avoid.
