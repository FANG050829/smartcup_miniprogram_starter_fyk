 ...
 * 等义移植自 mood-mates 的 engine.js
 * (github.com/sam70361/aora-bot 的 mood-mates 目录，
 *  社区许可：个人学习研究免费；本文件为面向微信小程序的重写)
 *
 * 移植差异（与本项目 emotion 引擎移植版同一套约定）：
 *   - 去除 window / document / DOM 查询；create 指向 Canvas 渲染层
 *   - rAF 由宿主注入（opts.raf，每引擎绑自己的 canvas），帧循环自调度
 *   - performance.now 缺失时回退 Date.now；Map/Set 换普通对象
 *   - 渲染帧异常逐帧拦截并上报 bootLog，绝不允许打断 rAF 链
 * 职责与算法与原版完全一致：
 *   角色注册中心 / 表情注册中心（语义槽位 + @色板令牌）/ rAF 状态机 /
 *   动画原语 / 弹簧插值 / 眼环·嘴环形变 / 眨眼 / 待机策略 / 巡演 / 点击庆祝
 * ============================================================ */
(function () {
  'use strict';
  ...
  var matesRenderer = require("./renderer.js");
  ...
  var nowFn = (typeof performance !== 'undefined' && performance.now && performance.now.bind(performance))
    ? function () {
         ...
         * 全链污染（同 emotion 引擎的防线），非有限数值一律回退 Date.now */
  ...
  /* glance 动画用的双曲正切：老内核缺 Math.tanh 时用有理近似兜底 */
  var tanhFn = (typeof Math.tanh === 'function')
    ? Math.tanh.bind(Math)
    : function (x) {
        if (x > 20) return 1;
        if (x < -20) return -1;
  ...
  function rgbToHex(r, g, b) {
    /* 不用 padStart：ES2017 方法，老内核真机会 TypeError */
    function hex2(v) {
  ...
  var MM = {
    ...
    characters: {
      register: function (raw) {
        ...
        if (!characters[raw.id]) charOrder.push(raw.id);
        characters[raw.id] = raw;
        return { ok: true, id: raw.id };
      },
      get: function (id) {
        var raw = characters[id];
        return raw ? resolveCharacter(raw) : null;
      },
      list: function () {
        return charOrder.map(function (id) { return resolveCharacter(characters[id]); });
      },
  ...
  var ANIM_TYPES = {
    ...
    glance: function (a, t) {
      ...
      return a.amp * tanhFn(2.8 * Math.sin(ph));
    },
  ...
  function validate(raw) {
    ...
    if (raw.pool != null) {
      /* 槽位名允许角色自定义轮廓（eyeShapes），未知名字在 normalizeFor 兜底 */
  ...
  function register(raw) {
    ...
    if (!registry[raw.id]) order.push(raw.id);
    registry[raw.id] = raw;
    configVersion++;
    return { ok: true, id: raw.id };
  }
  ...
  MM.config = {
    register: register,
    getRaw: function (id) { return registry[id] || null; },
    list: function (group) {
  ...
  function reportError(tag, err) {
    try { require("../bootLog.js").reportError(tag, err); } catch (e) {}
  }
  ...
  function Engine(surface, opts) {
    ...
    var ch = MM.characters.get(chId);
    if (!ch) throw new Error('[MoodMates] 未注册任何角色');
    this.character = ch;

    this.ball = matesRenderer.createBall(surface, Object.assign({}, opts, {
      character: ch,
  ...
  Engine.prototype = {
    ...
    _getDef: function (id) {
      ...
      if (this._defs[id]) return this._defs[id];
    ...
    setActive: function (on) {
      ...
      /* 未注入 raf 时不启动帧循环（快照引擎即此形态，静态渲染按需触发） */
    },
    ...
    /* 旋转/分屏 resize 透传：renderer 闭包内的映射常量重算（无入参） */
    resize: function () {
    ...
    /** 静态渲染一帧（快照管线入口：未激活时 setEmotion 自动走到这里） */
    renderStatic: function () {
      ...
      var seq = this._seq;
      this._seq = null;
      /* setEmotion 的同步首帧：异常就地拦截，不向调用方扩散 */
      try {
        this._tick(nowFn());
      } catch (err) {
        reportError("mates:setEmotion首帧异常(已拦截)", err);
      }
      this._seq = seq;
    },
    destroy: function () {
  ...
  MM.create = function (surface, opts) { return new Engine(surface, opts); };

  /* 载入角色与种子表情配置 */
  CHARACTERS.forEach(function (raw) {
    var r = MM.characters.register(raw);
    if (!r.ok) console.warn('[MoodMates] 角色注册无效：', r.id, r.errors);
  });
